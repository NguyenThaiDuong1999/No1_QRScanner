...
import com.scanqr.qrscanner.qrgenerator.databinding.ActivitySettingNo1Binding
import com.scanqr.qrscanner.qrgenerator.feature.language.LanguageNo1Activity
import com.scanqr.qrscanner.qrgenerator.helper.SharePrefHelper
import com.scanqr.qrscanner.qrgenerator.rate.RatingDialogNo1
import com.scanqr.qrscanner.qrgenerator.rate.SharePrefUtilsNo1
...
class SettingNo1Activity : BaseNo1Activity<ActivitySettingNo1Binding>() {
    ...
    override fun initData() {
        ...
        binding.llRate.tap {
            val dialog =
                RatingDialogNo1(this)
            dialog.init(this, object : RatingDialogNo1.OnPress {
                override fun send(rate: Float) {
                    ...
                    SharePrefUtilsNo1.forceRated(this@SettingNo1Activity)
                    Handler(mainLooper).postDelayed({
                        if (SharePrefUtilsNo1.isRated(this@SettingNo1Activity)) {
                            binding.llRate.gone()
                        } else {
                        ...
                        request.addOnCompleteListener { task ->
                            if (task.isSuccessful) {
                                ...
                                flow.addOnSuccessListener {
                                    SharePrefUtilsNo1.forceRated(activity)
                                    dialog.dismiss()
                                    Handler(mainLooper).postDelayed({
                                        if (SharePrefUtilsNo1.isRated(this@SettingNo1Activity)) {
                                            binding.llRate.gone()
    ...
    override fun onResume() {
        super.onResume()
        if (SharePrefUtilsNo1.isRated(this)) {
            binding.llRate.gone()
        } else {
            binding.llRate.visible()
        }
