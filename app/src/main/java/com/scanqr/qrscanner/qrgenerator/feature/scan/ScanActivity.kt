...
import com.scanqr.qrscanner.qrgenerator.databinding.ActivityScanBinding
...
import com.scanqr.qrscanner.qrgenerator.feature.scan.batch.ScanBatchResultActivity
import com.scanqr.qrscanner.qrgenerator.helper.SharePrefHelper
import com.scanqr.qrscanner.qrgenerator.helper.VibrateHelper
...
import com.scanqr.qrscanner.qrgenerator.utils.Constants.ScreenKey.FACEBOOK_SIGN
import com.scanqr.qrscanner.qrgenerator.utils.Constants.ScreenKey.INSTAGRAM_SIGN
import com.scanqr.qrscanner.qrgenerator.utils.Constants.ScreenKey.TWITTER_SIGN
import com.scanqr.qrscanner.qrgenerator.utils.Constants.ScreenKey.WHATSAPP_SIGN
import com.scanqr.qrscanner.qrgenerator.utils.Constants.ScreenKey.YOUTUBE_SIGN
...
class ScanActivity : BaseActivity<ActivityScanBinding>() {
    ...
    private var scanType = ScanType.SINGLE
    ...
    override fun initView() {
        ...
        startScanner { barcodes ->
            barcodes.forEach { barcode ->
                if (SharePrefHelper(this).getBoolean(IS_VIBRATE)) {
                    VibrateHelper(this).startVibrate()
                }
                if (SharePrefHelper(this).getBoolean(IS_SOUND)) {
                    mediaPlayer = MediaPlayer.create(this@ScanActivity, R.raw.scan_beep)
                    mediaPlayer?.start()
                }
        ...
        binding.icNextAfterBatch.tap {
            val intent = Intent(this, ScanBatchResultActivity::class.java)
            val bundle = Bundle()
    ...
    override fun getViewBinding(): ActivityScanBinding {
    ...
    private fun checkScanType(onScanSingle: () -> Unit, onScanBatch: () -> Unit) {
        when (scanType) {
            ScanType.SINGLE -> onScanSingle.invoke()
            ScanType.BATCH -> onScanBatch.invoke()
        }
    }
